import torch
import numpy as np
from rl_utils import detach_distribution, vector_to_parameters
...
def surrogate_loss(task, memory, model, old_pi=None):
    model.decoder_model.reset_context()
    model.adapt(task, reset=False)
    ctx = model.decoder_model.parameters_all[:-1]

    with torch.set_grad_enabled(old_pi is None):
        obs, action, logprob, reward, mask = memory.sample()
        pi = model.decoder_model(torch.from_numpy(np.stack(obs, axis=1)).float(), ctx_=ctx)
...
def hessian_vector_product(model, kl, damping=1e-2):
    grads = torch.autograd.grad(kl, model.decoder_model.parameters_all[2](), create_graph=True)
    flat_grad_kl = parameters_to_vector(grads)

    def _product(vector, retain_graph=True):
        grad_kl_v = torch.dot(flat_grad_kl, vector)
        grad2s = torch.autograd.grad(grad_kl_v, model.decoder_model.parameters_all[2](), retain_graph=retain_graph)
...
def trpo_optimization(memories, model):
    ...
    old_params = parameters_to_vector(model.decoder_model.parameters_all[2]())

    # Line search
    step_size = 1.0
    for _ in range(10):
        vector_to_parameters(old_params - step_size * step, model.decoder_model.parameters_all[2]())
        ...
        vector_to_parameters(old_params, model.decoder_model.parameters_all[2]())
