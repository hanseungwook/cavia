...
def run(args, log, tb_writer):
    ...
    weight = torch.load("./logs/1990.pth")
    ...
    train_data, val_data = get_data(task_family, args, number=1000)
    ...
    # Log result
    log[args.log_name].info("meta-loss: {:.3f}".format(meta_loss.detach().cpu().numpy()))
    ...
    vis_context(lower_contexts, task_family, 0, args)
