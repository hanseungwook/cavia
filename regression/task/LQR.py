...
def make_AB(x_dim, kbm): # = (0.,0.,0.)):
    ...
    A_kbm = x_dim*[x_dim*[0]]; A_kbm[-1] = list(kbm)[:x_dim]       # j + ma + bv + kx = F  #   k, b, m = kbm 
...
class Linear_Policy(nn.Module):
    ...
    def forward(self, obs, t):  # action = function ( observation, time )
        action = self.ctx_coeff[:1] @ ( obs[:1,:] - self.ctx_target )   #         action = self.coeff @ ( x - self.target ) 
        if self.ctx_coeff.shape[0]>1:
            action += self.ctx_coeff[1:] @ obs[1:,:]
        return action
...
class Combine_NN_ENV(nn.Module):
    def __init__(self, policy = None, x_dim=2, batch_nums = (1,3,1), kbm_zero = True, random_task = False, T = 20, traj_opt = False, x_range = 4):
        super().__init__()
    ...
    def forward(self, task, record = True): #:
        ...
        for t in range(self.T):
            ...
            if record:  ## Change to logger.update()
                x_all.append(obs.detach().view(-1))
