...
def run_no_inner(args, log_interval=5000, rerun=False):
    ...
    model = Encoder_Decoder(model_decoder, n_context=args.num_context_params, n_task=args.tasks_per_metaupdate)
...
def eval_1hot(args, model, task_family, num_updates, n_tasks=25, return_gradnorm=False):
    ...
    for t in range(n_tasks):
        # Get 1hot labels of total input range
        input_range_1hot = task_family.sample_task_onehot(num_tasks=n_tasks, batch_size=input_range.shape[0], cur_task=t)
        ...
        curr_inputs = task_family.sample_inputs(args.k_shot_eval, args.use_ordered_pixels).to(args.device)
        curr_1hot = task_family.sample_tasks_onehot(num_tasks=n_tasks, batch_size=args.k_shot_eval)
        ...
        for _ in range(1, num_updates + 1):
            # forward pass
            curr_outputs = model(curr_inputs, curr_1hot[t])
